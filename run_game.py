...
def parse_args():
    ...
    # Required arguments
    parser.add_argument("experiment_method", type=str, choices=['classic', 'exploration', 'discovery'], help="Method of the experiment. Supported: 'classic', 'exploration', 'discovery'.")
    ...
    parser.add_argument("--max_rounds", type=int, default=10, help="The maximum number of rounds to play. Currently only valid for DiscoveryGame. (default: 10).")
...
def setup(args) -> "Game":
    ...
    if args.experiment_method == "discovery":
        game_args["num_characters"] = args.num_characters
        game_args["max_rounds"] = args.max_rounds
        game = build_discovery(**game_args)
        game_created = True
